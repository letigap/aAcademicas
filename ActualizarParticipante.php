...
//VALIDAR EL PARAMETRO
 ...
 if (isset($_POST['agregar']) && !empty($_POST['agregar'])) {         
    ...
    $sql="SELECT * FROM participante WHERE IdParticipante = '$id'";
    $resultado=mysqli_query($dbc,$sql);
     foreach($resultado as $row): 
         
    //PRESENTACIÓN - VISTA
    ?>
    <h2 class="p-2 text-center">Actualizar datos de participante</h2>
	...
	<form class="form-horizontal" method="POST" action="ActualizarParticipante.php" autocomplete="off">
		<div class="form-group">
			...
			<div class="col-sm-10">
				<input type="text" class="form-control <?= (isset($errores['NombreParticipante'])) ? 'is-invalid' : '' ?>" id="NombreParticipante" name="NombreParticipante" placeholder="NombreParticipante" value="<?php echo $row['NombreParticipante']; ?>" required>
	...
	<input type="hidden" id="id" name="id" value="<?php echo $row['IdParticipante']; ?>" />
		...
		<div class="form-group">
			<label for="ApellidopParticipante" class="col-sm-2 control-label">Apellido Paterno</label>
				<div class="col-sm-10">
					<input type="text" class="form-control <?= (isset($errores['ApellidopParticipante'])) ? 'is-invalid' : '' ?>" id="ApellidopParticipante" name="ApellidopParticipante" placeholder="Apellido Paterno" value="<?php echo $row['ApellidopParticipante']; ?>" required>
		...
		<div class="form-group">
			<label for="ApellidomParticipante" class="col-sm-2 control-label">Apellido Materno</label>
				<div class="col-sm-10">
					<input type="text" class="form-control <?= (isset($errores['ApellidomParticipante'])) ? 'is-invalid' : '' ?>" id="ApellidomParticipante" name="ApellidomParticipante" placeholder="Apellido Materno" value="<?php echo $row['ApellidomParticipante']; ?>">
                    <div class="invalid-feedback"><span>
                        <?php
		...
		<div class="form-group">
			<label for="RfcParticipante" class="col-sm-2 control-label">RFC</label>
				<div class="col-sm-10">
					<input type="RfcParticipante" class="form-control<?= (isset($errores['RfcParticipante'])) ? 'is-invalid' : '' ?>" id="RfcParticipante" name="RfcParticipante" placeholder="RfcParticipante" value="<?php echo $row['RfcParticipante']; ?>"  required>
                    <div class="invalid-feedback">
                        <span>
                            <?php
		...
		<div class="form-group">
			<label for="EmailParticipante" class="col-sm-2 control-label">Email</label>
				<div class="col-sm-10">
					<input type="EmailParticipante" class="form-control <?= (isset($errores['EmailParticipante'])) ? 'is-invalid' : '' ?>" id="EmailParticipante" name="EmailParticipante" placeholder="Email" value="<?php echo $row['EmailParticipante']; ?>"  required>
                    <div class="invalid-feedback"><span>
                        <?php
		...
		<div class="form-group">
			<div class="col-sm-offset-2 col-sm-10">
				...
				<button type="submit" name="agregar" value="agregar" class="btn btn-primary">Guardar</button>
			</div>
		</div>
