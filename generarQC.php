...
<!--  <p align="center">A cargo de: '.$row['ProfesorEvento'].''.$row['ProcedenciaProfeEvento'].'.</p>-->
  ...
  <div class="posicion-num-certificado">
    ...
    <p class="right small">UNAM-DEPFE-'.$fecha.'-'.$row['IdParticipante'].'</p>
  </div>
  <div class="posicion-qr">
    <img class="qr" src="CONST_2021r/'.$row['IdParticipante'].''.$row['RfcParticipante'].'.png" >
  </div>
	
');
$mpdf -> WriteHTML('</body>');
$mpdf -> Output('Const-'.$row['IdParticipante'].'.pdf', 'F');
$mpdf->Output('CONST_2021/Const-'.$row['IdParticipante'].'.pdf', \Mpdf\Output\Destination::FILE);
